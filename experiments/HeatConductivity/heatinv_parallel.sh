#!/bin/bash
# Parallel launch for HeatInv RL optimizer chain.

SCRIPT="experiments/HeatConductivity/heatinv_chain.py"
TIMEOUT_SECONDS="${HEATINV_TIMEOUT_SECONDS:-42000}" # 11 hours 40 minutes
...
if ! [[ "$TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]]; then
    echo "HEATINV_TIMEOUT_SECONDS must be a positive integer, got: $TIMEOUT_SECONDS" >&2
    exit 2
fi
...
on_timeout() {
    echo "Time limit reached after ${TIMEOUT_SECONDS}s. Stopping all HeatInv processes..."
    stop_timer
    shutdown_children
    exit 124
}
...
on_interrupt() {
    echo "Launcher interrupted. Stopping all HeatInv processes..."
    stop_timer
    shutdown_children
    exit 130
}
