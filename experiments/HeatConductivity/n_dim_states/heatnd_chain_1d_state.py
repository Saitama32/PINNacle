...
TARGET_PROJECT_NAME = "rlpinn_heatnd_rebuild_buffer_1_dim"
DEFAULT_SOURCE_PROJECT_NAME = "rlpinn-heatnd-loss-chain-tolerance"
LOCAL_OUTPUT_DIR = os.path.join("transitions_rebuilt", "heatnd_1_dim")


def build_get_model_heatnd(hidden_layers: str, **pde_kwargs):
    def get_model():
        pde = HeatND(**pde_kwargs)
...
def main():
    ...
    parser.add_argument("--name", type=str, default="heatnd_1d_state_rebuild")
    ...
    experiment.log_parameters({
        ...
        "reward_function": "v_2",
        "description": (
            "rebuild_heatnd_buffer_1d_state"
            if args.rebuild_buffer_only
            else "optimization_heatnd_rl_optimizer_1d_state"
        ),
    ...
    get_model_rec = build_get_model_heatnd(args.hidden_layers, **pde_kwargs)
