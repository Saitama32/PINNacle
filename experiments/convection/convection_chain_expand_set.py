...
experiment = start_comet_experiment(
    project_name="rlpinn-convection-beta50-optimization_expand_set",
)
...
def build_get_model_convection(
    ...
    def get_model():
        pde = Convection1D(beta=beta, geom=geom, time=time_domain)
...
def main():
    ...
    pde_kwargs = {
        ...
        "time_domain": (args.t_min, args.t_max),
    }
    get_model = build_get_model_convection(args.hidden_layers, **pde_kwargs)
    get_model_rec = build_get_model_convection(args.hidden_layers, **pde_kwargs)
