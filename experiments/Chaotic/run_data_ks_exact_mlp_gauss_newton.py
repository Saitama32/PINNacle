...
import numpy as np
import torch
from deepxde.optimizers.pytorch.error_whitening_gn import ErrorWhiteningGN
...
def error_whitening_residual_closure(
    student,
    point_tensor,
    indices,
    target_tensors,
    ...
    """Build a fixed-batch closure returning the weighted residual vector."""

    def append_weighted(residual_parts, error, weight, scale=1.0):
        if weight <= 0.0:
            return
    ...
    def evaluate(create_graph_for_backward):
        ...
        append_weighted(
            residual_parts,
            predicted["u"] - target_tensors["u"][indices],
            weights["u"],
            scales["u"],
        )
        for key in active_derivatives:
            append_weighted(
                residual_parts,
                predicted[key] - target_tensors[key][indices],
                derivative_factor * weights[key],
                scales[key],
            )
        if pde_weight > 0.0:
            append_weighted(
                residual_parts, predicted["residual"], pde_weight
            )
...
def build_training_optimizer(student, args):
    ...
    if args.optimizer == "error_whitening_gn":
        dde.optimizers.set_ERROR_WHITENING_GN_options(
            rank=args.gn_rank,
            oversketch=args.gn_oversketch,
            tolerance=args.gn_tol,
...
def train_student(args, student, points, targets, scales, device, run_dir, metadata):
    ...
    scheduler = (
        ...
        if args.lr_min < args.lr and not isinstance(optimizer, ErrorWhiteningGN)
        else None
    )
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        if isinstance(optimizer, ErrorWhiteningGN):
            ...
            component_losses = {
                key: value.detach() for key, value in component_losses.items()
            }
            del predicted, data_error, batch_points
            residual_closure = error_whitening_residual_closure(
                student,
                point_tensor,
                indices,
                target_tensors,
            ...
            gn_diagnostics = optimizer.step(residual_closure)
            grad_norm = gn_diagnostics["grad_norm"]
        else:
            total.backward()
        ...
        if iteration == 1 or iteration % args.log_every == 0 or iteration == args.iterations:
            ...
            if gn_diagnostics is not None:
                row.update(gn_diagnostics)
            history.append(row)
            message = (
            ...
            if gn_diagnostics is not None:
                message += (
                    f" gn_loss={row['loss_before']:.3e}->{row['loss_after']:.3e}"
                    f" alpha={row['accepted_step_size']:.3e}"
                    f" rank={row['effective_rank']}"
                    f" explained_g={row['explained_gradient_fraction']:.3e}"
                )
            print(message)

    save_student_checkpoint(run_dir / "weights_student.pt", student, metadata)
    if isinstance(optimizer, ErrorWhiteningGN) and optimizer.diagnostics_history:
        gn_columns = list(optimizer.diagnostics_history[0])
        np.savetxt(
            run_dir / "error_whitening_gn_history.csv",
            np.asarray(
                [
                    [diagnostics[key] for key in gn_columns]
                    for diagnostics in optimizer.diagnostics_history
                ]
            ),
            delimiter=",",
            header=",".join(gn_columns),
            comments="",
        )
    if history:
...
def run(args):
    ...
    if args.gn_damping < 0.0 or not math.isfinite(args.gn_damping):
        raise ValueError("gn-damping must be finite and non-negative")
    if args.optimizer == "error_whitening_gn" and args.precision != "float64":
        raise ValueError("ErrorWhiteningGN requires --precision float64")
    if args.optimizer == "error_whitening_gn" and args.weight_decay != 0.0:
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--out", default=str(PROJECT_ROOT / "runs_data_ks_exact_mlp_gauss_newton")
    )
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "kl-m-soap", "madam", "muown", "error_whitening_gn",
        ],
        default="error_whitening_gn",
    )
