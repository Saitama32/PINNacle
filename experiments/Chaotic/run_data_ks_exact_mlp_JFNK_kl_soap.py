...
class JFNKKLSoap(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        if (
            ...
            _, no_prec_iterations, _, _, _, _ = self._pcg(
        ...
        if iteration == 1 or iteration % args.log_every == 0 or iteration == args.iterations:
            ...
            if jfnk_diagnostics is not None:
                message += (
                    ...
                    f"pcg={jfnk_diagnostics['cg_iterations']}/"
                    f"{args.jfnk_cg_max_iter} "
                    f"pcg_rel={jfnk_diagnostics['cg_relative_residual']:.3e} "
...
def parse_args(argv=None):
    ...
    parser.add_argument("--jfnk-damping", type=float, default=1e-4)
    parser.add_argument("--jfnk-cg-max-iter", type=int, default=20)
