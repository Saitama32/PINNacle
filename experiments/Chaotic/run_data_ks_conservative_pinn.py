"""Train the conservative two-field KS system without supervised data losses.

The network predicts ``(u, J)`` and minimizes conservation, flux compatibility,
and the initial condition. Reference data are used only for normalization,
...
from src.pde.chaotic import (
    ConservativeKuramotoSivashinskyEquation,
    build_conservative_ks_reference,
)
...
def conservative_terms(network, points, alpha, beta, gamma, backward=False):
    fields = network(points)
    u, flux = fields[:, 0:1], fields[:, 1:2]
    first_u = torch.autograd.grad(
        u, points, grad_outputs=torch.ones_like(u), create_graph=True
    )[0]
    u_x, u_t = first_u[:, 0:1], first_u[:, 1:2]
    ...
    flux_x = torch.autograd.grad(
        flux, points, grad_outputs=torch.ones_like(flux), create_graph=backward
    )[0][:, 0:1]
...
def evaluate_residuals(network, bounds, pde, args, device):
    ...
    sums = {"conservation": 0.0, "flux_compatibility": 0.0}
...
def save_plot(path, points, exact, prediction, title):
    ...
    for values in (exact, prediction, np.abs(prediction - exact)):
        ...
        field[ti, xi] = values
        arrays.append(field)
    low, high = min(float(np.min(exact)), float(np.min(prediction))), max(
        float(np.max(exact)), float(np.max(prediction))
    )
    ...
    for axis, image, label in zip(axes, images, ("Exact u", "Flux PINN u", "Absolute error")):
...
def validate(args):
    ...
    for name in ("conservation_weight", "flux_weight", "ic_weight"):
        value = getattr(args, name)
        if not math.isfinite(value) or value < 0.0:
            raise ValueError(f"{name.replace('_', '-')} must be finite and non-negative")
    if args.conservation_weight + args.flux_weight + args.ic_weight <= 0.0:
...
def train(args, network, bounds, pde, reference, device, run_dir, metadata):
    ...
    numpy_dtype = np.float64 if dtype == torch.float64 else np.float32
    lower, upper = np.asarray(bounds[0], dtype=numpy_dtype), np.asarray(bounds[1], dtype=numpy_dtype)
    optimizer = build_optimizer(network, args)
    scheduler = (
        torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.iterations, eta_min=args.lr_min)
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        conservation_mse = torch.mean(terms["conservation"].square())
        flux_mse = torch.mean(terms["flux_compatibility"].square())
        ...
        exact_ic = torch.as_tensor(np.cos(x) * (1.0 + np.sin(x)), dtype=dtype, device=device)
        ic_mse = torch.mean((network(ic_points)[:, 0:1] - exact_ic).square())
        total = (
            args.conservation_weight * conservation_mse
            + args.flux_weight * flux_mse
        ...
        if iteration == 1 or iteration % args.log_every == 0 or iteration == args.iterations:
            ...
            u_metric = metrics(prediction[:, 0], reference[:, 2])
            flux_metric = metrics(prediction[:, 1], reference[:, 3])
            row = {
                "iteration": iteration,
                "loss_total": float(total.detach().cpu()),
                "conservation_mse": float(conservation_mse.detach().cpu()),
                "flux_compatibility_mse": float(flux_mse.detach().cpu()),
                "ic_mse": float(ic_mse.detach().cpu()),
                "u_reference_l2re": u_metric["relative_l2"],
                "flux_reference_l2re": flux_metric["relative_l2"],
            ...
            print(
                ...
                f"conservation_mse={row['conservation_mse']:.6e} "
                f"flux_mse={row['flux_compatibility_mse']:.6e} "
                f"ic_mse={row['ic_mse']:.6e} u_l2re={row['u_reference_l2re']:.6e} "
                f"flux_l2re={row['flux_reference_l2re']:.6e}"
            )
...
def run(args):
    ...
    pde = ConservativeKuramotoSivashinskyEquation(datapath=args.data, bbox=bbox)
    reference = build_conservative_ks_reference(
        physical, pde.alpha, pde.beta, pde.gamma
    )
    points, fields = reference[:, :2], reference[:, 2:4]
    lower, upper = np.asarray([bbox[0], bbox[2]]), np.asarray([bbox[1], bbox[3]])
    network, metadata = build_network(args, points, fields, device)
    metadata.update(
        training_objective="conservative_ks_residuals_and_initial_condition",
        coefficients={"alpha": pde.alpha, "beta": pde.beta, "gamma": pde.gamma},
        loss_weights={
            "conservation": args.conservation_weight,
            "flux_compatibility": args.flux_weight,
            "ic": args.ic_weight,
            "data": 0.0,
            "sobolev": 0.0,
        },
    ...
    run_dir = Path(args.out).expanduser().resolve() / (
        f"{timestamp}-ks-conservative-pinn-{args.network}-{args.optimizer}"
    )
    ...
    history = train(args, network, (lower, upper), pde, reference, device, run_dir, metadata)
    ...
    u_metric, flux_metric = metrics(prediction[:, 0], fields[:, 0]), metrics(prediction[:, 1], fields[:, 1])
    np.savez_compressed(
        run_dir / "predictions.npz", x=points[:, 0], t=points[:, 1],
        exact_u=fields[:, 0], prediction_u=prediction[:, 0],
        exact_flux=fields[:, 1], prediction_flux=prediction[:, 1],
    )
    save_plot(
        run_dir / "solution.png", points, fields[:, 0], prediction[:, 0],
        f"Conservative KS PINN, u L2RE={u_metric['relative_l2']:.3e}",
    )
    result = {
        **residuals, "initial_condition": ic_metric, "u_reference": u_metric,
        "flux_reference": flux_metric, "last_training_row": history[-1],
        "configuration": configuration,
    }
    with (run_dir / "metrics.json").open("w", encoding="utf-8") as handle:
        json.dump(result, handle, indent=2, sort_keys=True)
    print(
        f"Conservative KS PINN: conservation MSE={residuals['conservation_mse']:.6e}; "
        f"flux MSE={residuals['flux_compatibility_mse']:.6e}; "
        f"IC MSE={ic_metric['mse']:.6e}; u L2RE={u_metric['relative_l2']:.6e}; "
        f"flux L2RE={flux_metric['relative_l2']:.6e}; artifacts={run_dir}"
    )
    return run_dir
...
def parse_args(argv=None):
    ...
    parser.add_argument("--out", default=str(PROJECT_ROOT / "runs_data_ks_conservative_pinn"))
    ...
    parser.add_argument("--optimizer", choices=["adam", "rmsprop", "madam", "soap", "kl-m-soap", "rekls-v3", "muown"], default="rekls-v3")
    parser.add_argument("--lr", type=float, default=5e-4)
    parser.add_argument("--lr-min", type=float, default=5e-4)
    parser.add_argument("--weight-decay", type=float, default=0.0)
    parser.add_argument("--conservation-weight", type=float, default=1.0)
    parser.add_argument("--flux-weight", type=float, default=1.0)
