#!/bin/bash
# Parallel launch for Kuramoto-Sivashinsky RL optimizer chain.

SCRIPT="experiments/Chaotic/ks_sobolev_chain.py"
TIMEOUT_SECONDS="${KS_TIMEOUT_SECONDS:-42000}" # 11 hours 40 minutes
...
on_timeout() {
    echo "Time limit reached after ${TIMEOUT_SECONDS}s. Stopping all KS processes..."
    stop_timer
    shutdown_children
    exit 124
}
...
on_interrupt() {
    echo "Launcher interrupted. Stopping all KS processes..."
    stop_timer
    shutdown_children
    exit 130
}
