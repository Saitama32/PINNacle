#!/bin/bash
# Auto-run one Poisson3D complex-geometry ablation and distribute two jobs across available GPUs.
...
run_job() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python "$SCRIPT" \
        --ablation "$ABLATION" \
        --name "poisson3d_complexgeometry_${ABLATION}_replica_${replica}" &
}
