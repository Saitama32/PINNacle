...
experiment = start(
    api_key=api_key,
    project_name=f"rlpinn_poisson_nd_{_state_type_for_comet}_comparison",
    workspace="saitama32",
)
...
def build_get_model_poisson_nd(hidden_layers: str, **pde_kwargs):
    def get_model():
...
def main(seed_override=None):
    ...
    parser.add_argument("--name", type=str, default="poisson_nd_rl")
    ...
    experiment.log_parameters({
        "param": "v_1",
        "reward_function": "v_2",
        "description": "comparison_poisson_nd_loaded_dqn_final_eval",
    ...
    pde_kwargs = {"dim": args.dim, "len": args.length}
    get_model = build_get_model_poisson_nd(args.hidden_layers, **pde_kwargs)
    get_model_rec = build_get_model_poisson_nd(args.hidden_layers, **pde_kwargs)
