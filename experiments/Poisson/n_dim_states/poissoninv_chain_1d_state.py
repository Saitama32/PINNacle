...
from src.pde.inverse import PoissonInv
...
TARGET_PROJECT_NAME = "rlpinn_poissoninv_rebuild_buffer_1_dim"
DEFAULT_SOURCE_PROJECT_NAME = "rlpinn-poissoninv-loss-chain-tolerance"
LOCAL_OUTPUT_DIR = os.path.join("transitions_rebuilt", "poissoninv_1_dim")


def build_get_model_poissoninv(hidden_layers: str):
    def get_model():
        pde = PoissonInv()
...
def main():
    ...
    parser.add_argument("--name", type=str, default="poissoninv_1d_state_rebuild")
    ...
    experiment.log_parameters({
        ...
        "reward_function": "v_2",
        "description": (
            "rebuild_poissoninv_buffer_1d_state"
            if args.rebuild_buffer_only
            else "optimization_poissoninv_rl_optimizer_1d_state"
        ),
    ...
    get_model = build_get_model_poissoninv(args.hidden_layers)
    get_model_rec = build_get_model_poissoninv(args.hidden_layers)
