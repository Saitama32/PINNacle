#!/bin/bash
# Parallel launch for the Gray-Scott RL optimizer chain.

SCRIPT="experiments/GrayScott/grayscott_chain.py"
TIMEOUT_SECONDS="${GRAYSCOTT_TIMEOUT_SECONDS:-42000}" # 11 hours 40 minutes
KILL_GRACE_SECONDS=30
PIDS=()
TIMER_PID=""

if ! [[ "$TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]]; then
    echo "GRAYSCOTT_TIMEOUT_SECONDS must be a positive integer, got: $TIMEOUT_SECONDS" >&2
    exit 2
fi
...
on_timeout() {
    echo "Time limit reached after ${TIMEOUT_SECONDS}s. Stopping all Gray-Scott processes..."
    stop_timer
    shutdown_children
    exit 124
}
...
on_interrupt() {
    echo "Launcher interrupted. Stopping all Gray-Scott processes..."
    stop_timer
    shutdown_children
    exit 130
}
...
echo "Gray-Scott processes: ${PIDS[*]}"
