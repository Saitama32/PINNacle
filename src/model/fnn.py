...
from .sfli import SFLIConfig, SFLIGaussianFirstLayer, apply_dense_sfli
...
class FNN(NN):
    ...
    def first_layer_features(self, inputs):
        ...
        return self.activation(first_preactivation)
    ...
    def forward(self, inputs):
        ...
        if self.sfli_type == "baseline" and len(self.linears) == 1:
            ...
            first_preactivation = self.linears[0](x)
            if self.sfli_type == "tanh":
                x = torch.tanh(first_preactivation)
            elif self.sfli_type == "cosine":
                x = torch.cos(first_preactivation)
            else:
                x = self.activation(first_preactivation)
